...
func tableGoogleworkspaceAdminReportsLoginActivity(ctx context.Context) *plugin.Table {
    return &plugin.Table{
        Name:        "googleworkspace_admin_reports_login_activity",
        Description: "Records of user sign‑in events.",

        List: &plugin.ListConfig{
            Hydrate: listGoogleworkspaceAdminReportsLoginActivities,
        ...
        Columns: []*plugin.Column{
            ...
            {
                Name:        "event_name",
                Description: "First event name for this activity",
                Type:        proto.ColumnType_STRING,
...
func listGoogleworkspaceAdminReportsLoginActivities(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
    service, err := ReportsService(ctx, d)
    if err != nil {
        plugin.Logger(ctx).Error("googleworkspace_admin_reports_login_activity.list", "service_error", err)
        return nil, err
    }
    ...
    for {
        ...
        resp, err := call.Do()
        if err != nil {
            plugin.Logger(ctx).Error("googleworkspace_admin_reports_login_activity.list", "api_error", err)
            return nil, err
        }
...
func extractFirstEventName(_ context.Context, d *transform.TransformData) (interface{}, error) {
    events, ok := d.Value.([]*admin.ActivityEvents)
    if !ok || len(events) == 0 {
        return "", nil
    }
