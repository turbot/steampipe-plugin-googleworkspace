...
func googleworkspaceDriveFileColumns(columns []*plugin.Column) []*plugin.Column {
	return append(columns, driveFileColumns()...)
}

//// TABLE DEFINITION

func tableGoogleWorkspaceDriveFile(_ context.Context) *plugin.Table {
	return &plugin.Table{
		Name:        "googleworkspace_drive_file",
		Description: "Retrieves metadata or contents of files shared to user's domain, or shared in specified shared drive.",
		List: &plugin.ListConfig{
			Hydrate: listDriveFiles,
		...
		Columns: googleworkspaceDriveFileColumns([]*plugin.Column{
			{
				Name:        "corpora",
				...
				Type:        proto.ColumnType_STRING,
				Transform:   transform.FromQual("corpora"),
			},
		}),
	}
}
...
func listDriveFiles(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
	// Create service
	...
	var includeTeamDriveItems, supportsAllDrives bool
	corpora := d.KeyColumnQuals["corpora"].GetStringValue()
	switch corpora {
	case "drive", "allDrives":
	...
	// Checks for files within specified shared drive
	var driveID string
	if d.KeyColumnQuals["drive_id"] != nil {
		driveID = d.KeyColumnQuals["drive_id"].GetStringValue()
	}

	// Use "*" to return all fields
	resp := service.Files.List().Fields("nextPageToken, files(*)").Q(query).DriveId(driveID).Corpora(corpora).IncludeTeamDriveItems(includeTeamDriveItems).SupportsAllDrives(supportsAllDrives)
