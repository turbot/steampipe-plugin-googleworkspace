...
func tableGoogleworkspaceAdminReportsTokenActivity(ctx context.Context) *plugin.Table {
    return &plugin.Table{
        Name:        "googleworkspace_admin_reports_token_activity",
        Description: "OAuth and API token events, such as grant authorizations and revocations.",

        List: &plugin.ListConfig{
            Hydrate: listGoogleworkspaceAdminReportsTokenActivities,
...
func listGoogleworkspaceAdminReportsTokenActivities(ctx context.Context, d *plugin.QueryData, _ *plugin.HydrateData) (interface{}, error) {
    service, err := ReportsService(ctx, d)
    if err != nil {
        plugin.Logger(ctx).Error("googleworkspace_admin_reports_token_activity.list", "service_error", err)
        return nil, err
    }
    ...
    for {
        ...
        resp, err := call.Do()
        if err != nil {
            plugin.Logger(ctx).Error("googleworkspace_admin_reports_token_activity.list", "api_error", err)
            return nil, err
        }
